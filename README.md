...
- **Green** (`chop me a beat`): drop one or several `.wav`, `.mp3`, `.ogg`,
  `.flac`, `.aiff`, or `.m4a` files — each becomes its own MIDI-pad-style
  4x4 block (up to 6 at a time) in Green's track. Each sample is sliced
  into 16 equal chunks, each chunk's loudness is analyzed and used to
  auto-build that pad block's pattern (louder/percussive slices are more
  likely to be turned on), and all pad blocks mix together as they loop.
  Click any pad to toggle that chunk on/off.
...
1.3.0.exe`) in the `dist/` folder. You'll need an internet connection the
...
anyone you hand `Turtle Setup 1.3.0.exe` to only needs to double-click it,
...
renderer.js   Everything else: turtle drawing/animation for all three
              ...
              and BPM detection, the shared step scheduler, Green's pad
              blocks, Blue's playlist + segment-mute bars, Red's
              segment-mute bars, and WAV export/rendering.
style.css     Look & feel — the pond scene (CSS gradients + lily
              pads/ripples/reeds), the custom titlebar, the always-visible
              track panels with per-turtle accent colors, Green's pad grid,
              Blue/Red's segmented bars.
...
- **Slice count**: change `STEP_COUNT` at the top of `renderer.js` (used
  by Green, Red, and Blue's mute segments; keep it a multiple of 4 so it
  stays musically sane. Green's `.padGrid` CSS assumes a 4x4 layout and
  Blue/Red's `.segBar` just flexes to however many `.seg` cells exist, so
  only the pad grid's `grid-template-columns` needs updating if you change
  it).
...
- **Max sounds per turtle**: `MAX_SOUNDS_PER_TURTLE` at the top of
  ...
  turtles' pad blocks/bars/playlist). Since all three tracks are always
  visible and `#tracks` scrolls as a whole, raising this doesn't need any
  CSS changes.
- **Green's pads**: `renderGreenGrid()` / `buildPadBlock()` in
  `renderer.js` build one `.padBlock` (a 4x4 `.padGrid`) per dropped
  sound — same underlying 16-step `pattern` array as before, just laid
  out square instead of in a row.
- **Blue/Red's segmented bars**: `buildSegBarRow()` in `renderer.js` is
  the shared builder for both — it renders whatever array you pass it
  (Red's `layer.pattern`, Blue's `track.mute`) as a row of clickable
  `.seg` cells and mutates that array in place on click.
