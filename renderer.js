...
// ---------------------------------------------------------------------------
// Green, Blue, and Red all share the same segmented-bar widget — one
// horizontal 16-segment bar per dropped sound. Clicking a segment toggles
// that piece. Green's and Red's segments are literally their step
// pattern (same data as before, consistent look now); Blue's segments
// mute pieces of an otherwise whole, uncut sample via the gain automation
// in scheduleBlueBuffer().
// ---------------------------------------------------------------------------
function renderGreenGrid() {
  ...
  if (!greenLayers.length) {
    ...
    msg.textContent = 'drop a sound on Green above to get a bar here';
    stepsGreenEl.appendChild(msg);
    return;
  }
  greenLayers.forEach((layer) => {
    stepsGreenEl.appendChild(
...
function detectBpm(buffer) {
  try {
    ...
    // Track the max alongside flux instead of Math.max(...flux) — spreading
    // a large typed array as call arguments can blow the call stack on
    // longer tracks (a several-minute drop is thousands of ~10ms windows).
    const flux = new Float32Array(numWindows);
    let maxFlux = 0;
    for (let w = 1; w < numWindows; w++) {
      const d = envelope[w] - envelope[w - 1];
      flux[w] = d > 0 ? d : 0;
      if (flux[w] > maxFlux) maxFlux = flux[w];
    }
    ...
    // A detection failure should never sink the whole drop — fall back to
    // "no tempo match" (playbackRate stays 1x) rather than losing the sound.
    console.error('detectBpm failed, falling back to no tempo-match:', err);
    return null;
  }
}
...
async function handleBlueDrop(audioFiles) {
  ...
  // Green and Red pick up newly-dropped sounds automatically because the
  // step scheduler re-reads their arrays every step. Blue doesn't work that
  // way — it's driven by an onended chain that only starts when explicitly
  // kicked off. So if playback is already running (from Green/Red, or from
  // Blue tracks dropped earlier) and Blue doesn't have anything actually
  // playing yet, kick it off right here instead of waiting on scheduleAutoStart
  // (which no-ops once `playing` is already true).
  if (playing && !blueActiveSource && blueQueue.length) {
    scheduleBlueTrack(blueCurrentIndex);
  }
